...
class Migrate
{
    ...
    public function __construct($config = [])
    {
        ...
        if ($this->database['adapter'] !== 'Mysql') {
            throw new \Exception('Only Support MySQL');
        }

        $this->_db = new Db($this->database);
    ...
    {
        $_migrations = array_slice($this->_getNew(), 0, $limit);
        $migrationTable = $this->getTableName($this->migrationTable);
        if(empty($_migrations)) {
            return "No effect";
        }
        $migrations = [];
        foreach ($_migrations as $migration) {
            $version = str_replace('.php','',basename($migration));
            $data = $this->_db->fetch("SELECT * FROM {$migrationTable} WHERE version='{$version}'");
            if(empty($data)) {
    ...
    private function _getHistory()
    {
        ...
        return $this->_db->fetchAll(
            "SELECT * FROM {$migrationTable} WHERE `version` <> ? ORDER BY `apply_time` ASC",
            [self::BASE_MIGRATION]
        );
    }
